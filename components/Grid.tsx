...
export default function Grid({
  ...
  useEffect(()=>{ onActiveChange?.({ number: activeNumber, dir }) }, [activeNumber, dir, onActiveChange])
  ...
  const highlighted = new Set(activeWord.map(c=>`${c.row}_${c.col}`))
  function toggleDir(){ setDir(d=> d==='across' ? 'down' : 'across') }
  ...
  return (
    <div>
      ...
      <div className="px-2">
        <div className="grid grid-cols-15 w-full border border-gray-300 dark:border-gray-700">
          {cells.map((cell, i)=>{
            ...
            return (
              <div
                id={`cell_${id}`}
                key={i}
                onClick={()=>handleTap(cell.row, cell.col)}
                className={clsx(
...
function getWordCells(cells: Cell[], start: {row:number,col:number}, dir: Direction){
  const out: Cell[] = []
  const step = dir==='across' ? (r:number,c:number)=>({r, c:c+1}) : (r:number,c:number)=>({r:r+1, c})
  let {row, col} = start
  while (true){
